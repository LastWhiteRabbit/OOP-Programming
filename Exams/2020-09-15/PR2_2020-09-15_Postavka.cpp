...
class Ispit {
    Predmet _predmet;
    //string se odnosi na rjesenje/odgovor ili napomenu nastalu tokom ispita
...
const char* GetOdgovorNaDrugoPitanje() {
    cout << "Pitanje -> Ukratko pojasnite primjenu funkcija seekp i tellp?\n";
    return "Odgovor -> OVDJE UNESITE VAS ODGOVOR";
}
void main() {
...
/*onemoguceno dupliranje podrazumijeva da obje vrijednosti para (clana kolekcije) moraju biti iste, a ne samo jedna (T1) ili druga (T2)*/
    ...
    try {
/*ukoliko dupliranje vrijednosti nije dozvoljeno (BEZ_DUPLIKATA) metoda AddElement baca izuzetak u slucaju da se pokusa dodati par sa vrijednostima        identicnim postojecem paru unutar kolekcije */
    ...
    /*na osnovu vrijednosti tip T2 (u primjeru vrijednost 1) pronalazi i vraca iz kolekcije sve parove
    koji kao vrijednost T2 imaju proslijedjenu vrijednost*/
    ...
    */
    ...
    if (jasminPolaznik != nullptr) {
        if (jasminPolaznik->AddPitanje(PRI, dinamickaMemorija, "moze se koristiti new i malloc"))
        ...
        if (jasminPolaznik->AddPitanje(PRI, visenitnoProgramiranje))
            cout << "Pitanje uspjesno dodano!" << crt;
        if (jasminPolaznik->AddPitanje(PRI, regex, "za validaciju emaila mozemo koristiti razlicite regex patterne kojih se trenutno ne mogu sjetiti"))
            cout << "Pitanje uspjesno dodano!" << crt;
        ...
        //vraca broj ponavljanja odredjene rijeci unutar odgovora/napomena nastalih tokom polaganja ispita.
        int brojPonavljanja = (*jasminPolaznik)("new");
        cout << "Rijec new se ponavlja " << brojPonavljanja << " puta." << endl;
...
/*ukoliko su prethodno kreirani i koristeni pokazivaci (jasmin, adel i lozinkaNijeValidna), nije dozvoljeno uklanjanje njihove dealokacije*/
    delete jasmin;
    delete adel;
